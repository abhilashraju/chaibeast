...
{
    return stdexec::just() |
           stdexec::then([&, handler = std::move(readHandler)]() {
               return handler(sock);
           });
}
...
struct Server
{
    std::string port;
    Router router;
    Server(const std::string_view& p, const std::string_view& dip,
           const std::string_view& dp, const std::string_view config) :
        port(p.data(), p.length()),
        router({dip.data(), dip.length()}, {dp.data(), dp.length()},
